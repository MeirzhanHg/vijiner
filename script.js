window.addEventListener('DOMContentLoaded', function() {
    ...
    var KazAlfUp = ['А', 'Ә', 'Б', 'В', 'Г', 'Ғ', 'Д', 'Е', 'Ё', 'Ж', 'З', 'И', 'Й', 'К', 'Қ', 'Л', 'М', 'Н', 'Ң', 'О', 'Ө', 'П', 'Р', 'С', 'Т', 'У', 'Ұ', 'Ү', 'Ф', 'Х', 'Һ', 'Ц', 'Ч', 'Ш', 'Щ', 'Ъ', 'Ы', 'І', 'Ь', 'Э', 'Ю', 'Я'];
    var KazAlfLower = ['а', 'ә', 'б', 'в', 'г', 'ғ', 'д', 'е', 'ё', 'ж', 'з', 'и', 'й', 'к', 'қ', 'л', 'м', 'н', 'ң', 'о', 'ө', 'п', 'р', 'с', 'т', 'у', 'ұ', 'ү', 'ф', 'х', 'һ', 'ц', 'ч', 'ш', 'щ', 'ъ', 'ы', 'і', 'ь', 'э', 'ю', 'я'];
    ...
    var KazAlfUpEncrypt = Array(42);
    var KazAlfLowerEncrypt = Array(42);
    ...
    function initEncrypt() {
      KazAlfUpEncrypt = CezarEncrypt(UserStep, KazAlfUp);
      KazAlfLowerEncrypt = CezarEncrypt(UserStep, KazAlfLower);
    ...
    function encrypt(text) {
      ...
      for (var i = 0; i <= text.length; i++) {
        ...
        //     result += symbol;
        // }
        if (contains(symbol, KazAlfUp)) {
            symbol = KazAlfUpEncrypt[pos];
            result += symbol;
        }
        if ((contains(symbol, KazAlfLower))) {
            symbol = KazAlfLowerEncrypt[pos];
            result += symbol;
        }
    ...
    function decrypt(text) {
      ...
      for (var i = 0; i <= text.length; i++) {
        ...
        //     result += symbol;
        // }
        if (contains(symbol, KazAlfUpEncrypt)) {
            symbol = KazAlfUp[pos];
            result += symbol;
        }
        if ((contains(symbol, KazAlfLowerEncrypt))) {
            symbol = KazAlfLower[pos];
            result += symbol;
        }
